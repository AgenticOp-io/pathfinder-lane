...
func findToolTipLayer(handle *ToolTipHandle, logErr bool) *ToolTipLayer {
	...
	if handle.overlay != nil {
		otl := tl.overlays[handle.overlay]
		if otl == nil {
			// Fyne menus use OverlayContainer; dialogs use *widget.PopUp but
			// callers rarely call AddPopUpToolTipLayer. Missing overlay tip
			// layers are expected — skip the tip quietly (do not LogError).
			return nil
		}
		return otl
	}
